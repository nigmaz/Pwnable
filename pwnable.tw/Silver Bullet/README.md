...
```
- Nó nằm ở câu lệnh `strncat(dest, tmp, 48 - *((_DWORD *)dest + 12));` hay theo cấu trúc được định nghĩa lại thì sẽ hiểu là `strncat(bullet.name, name, 0x30-bullet.power)` .
...
- Dẫn chứng trường hợp từ câu lệnh `strncat(dest, tmp, 48 - *((_DWORD *)dest + 12));` - mục đích là nối chuỗi `tmp` mà người dùng mới nhập vào dest là chuỗi `bullet.name`, số ký tự được copy tối đa là `0x30-bullet.power`.

- Giả sử lúc đầu bullet.name là một chuỗi gồm 0x2f kí tự ‘A’, tmp là chuỗi ‘B’ dài 0x1 kí tự. Sau khi thực hiện strncat() thì bullet.name sẽ gồm 0x30 kí tự, cộng thêm 1 kí tự NULL được strncat() tự động thêm vào bullet.name[48] = NULL - đó chính là vị trí của bullet.power. Và bullet.power sẽ được cập nhật lại thành `bullet.power = bullet.power + len(tmp) = 0 + 1 = 1.`
